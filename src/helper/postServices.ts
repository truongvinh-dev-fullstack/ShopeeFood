...
export const formatDate = (
  ...
  switch (type) {
    ...
    case 'Thu, dd/MM/YYYY':
      return `${getDayOfWeek(date)}, ${padTo2Digits(
        date.getDate(),
    ...
    case 'MM/DD/YYYY':
      return `${monthNames[date.getMonth()]}/${padTo2Digits(
        date.getDate(),
      )}/${date.getFullYear()}`;
...
export const getFullDayOfWeek = () => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    weekDates.push({
      ...
      date: `${padTo2Digits(currentDate.getDate())}/${padTo2Digits(
...
export const formatCurrency = (number: number | string) => {
  ...
  const numberString =
    typeof number === 'number' ? number.toFixed(2) : Number(number).toFixed(2);
...
// Hàm ánh xạ trả về số tương ứng, vd [100 , 150] trả về [1,0] thì truyền mapRange(100, 100, 150, 1, 0) sẽ ra 1
